...
class MagicardApplicationTests {
	...
	//
	// Request request =
	// requestRepo.findById(UUID.fromString("00fa39eb-b308-4fb2-963e-3977187bf813")).orElse(null);
	//
	...
	// List<Employee> superEmployees =
	// employeeRepo.findByDepartment(employee.getDepartment());
	// String superEmployeeEmail =
	// superEmployees.get(random.nextInt(superEmployees.size())).getEmployeeEmail();
	//
	//
	...
	// paymentInfoRepo.findById(UUID.fromString("35f86ed0-c7ef-55eb-bf10-b42e99073dcc")).orElse(null);
	// PurposeItem purposeItem =
	// purposeItemRepo.findById(UUID.fromString("f4b24d03-eb07-42b9-a4f4-292e49591c16")).orElse(null);
	//
	// Company company = employee.getEmployeeRank().getCompany(); // 나의 company 정보
	//
	...
	// // 해당 회사의 모든 회원 가져오기
	//
	// List<EmployeeRank> myCompanyEmployeeRank =
	// employeeRankRepo.findByCompany(company);
	// // 같은 회사 사람들 리스트
	//// List<Employee> companyEmployeeList = findByCompany(myCompanyEmployeeRank);
	//
	// Department department = employee.getDepartment();
	// List<Employee> sameDeptEmployees = findByDepartment(myCompanyEmployeeRank,
	// department);
	//
	...
	//
	// // 내가 상급자가 아니라면
	// if(employee.getEmployeeRank().getEmployeeRankId() !=
	// sameDeptEmpRankList.get(0)) {
	// // 내 상급자의 랭크는 get(0)
	...
	// List<Employee> superDeptEmployees = findByDepartment(myCompanyEmployeeRank,
	// dept);
	//
	...
	// List<Employee> findByDepartment(List<EmployeeRank> myCompanyEmployeeRank,
	// Department department){
	// List<Employee> sameDeptEmployees = new ArrayList<>();
	// for(EmployeeRank employeeRank : myCompanyEmployeeRank){
	// List<Employee> employees =
	// employeeRepo.findByEmployeeRankAndDepartment(employeeRank, department);
	...
	// String[][] itemArray = {{"급여", "4대보험", "식대", "포상금", "상여금", "경조사비"}, {"사무실 차임
	// 보증금"},
	...
	// // String[] categoryList = {"인건비","사업장 임대료", "인테리어 비용", "차량유지비", "통신비",
	// "활동비", "출장비", "비품 구입비", "접대비"};
