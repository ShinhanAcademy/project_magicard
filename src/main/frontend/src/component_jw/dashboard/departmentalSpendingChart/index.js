...
const superDepartments = [
  '경영부문','전략사업부문','디지털개발부문','인프라&보안부문','경영전략본부','글로벌본부','디지털본부','뱅킹Biz본부','정보보호본부','인프라본부'
];
...
const DepartmentalSpendingChart = () => {
  ...
  return (
    <Card>
      ...
      <Grid container spacing={2} sx={{ position: "relative", ml: "auto" }}  style={{ paddingLeft: '1%' }} >
        <Grid className="gridContainer" item xs={2.5} style={{paddingTop:'2.75rem'}} >
          <SoftTypography variant="h5" fontWeight="bold" 
            gutterBottom mt={3} mb={3}>
            부서 전체 지출 TOP5
          </SoftTypography>
          <List>
            {top5Departments.map((item, index) => (
            <ListItem key={index} className="listItem">
              <SoftTypography className="department-rank" variant="body2" color="text" fontWeight="medium">{index+1}</SoftTypography>
              <SoftTypography className="department-name" variant="body2" color="text" fontWeight="medium">{item.department}</SoftTypography>
              <SoftTypography className="spend-amount" variant="body2" color="text" fontWeight="medium">{item.spend} 만원</SoftTypography>
            </ListItem>
            ))}
          </List>
        </Grid>
